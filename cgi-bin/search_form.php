...
<b>Wildcards:</b><br />The following rules apply only when NOT using <a href="https://mariadb.com/kb/en/regular-expressions-overview/" target="_blank">MariaDB regular expressions</a>, however, it is useful to note the pattern and length limits for specific fields below when constructing regular expressions for those fields.<br />
<b>When using wildcards, the letters in your search pattern are automatically converted to all-capitals,</b> because the database contains only capital letters.<br />
To escape, precede each wildcard character that you wish to escape with a backslash, e.g. *\** matches any string with at least one asterisk.<br />* or % matches any string <b>(do not use this in the doc_id.)</b>  Will be converted to % in the query.<br />? matches any single character. Will be converted to _ in the query, but <b>do not use _ as a wildcard;</b> it will be converted to \_ in the query.<br /><b>doc_id</b> must be exactly 11 characters of form YYWWDUUUNNN where YY is the last 2 digits of the year, with A0-A9 for 2000-2009, B0-B9 for 2010-2019, etc; WW is the week (01-53), D is the day of the week (1-7), UUU is the username (3 characters), and NNN is the document number (3 characters). ? may substitute for any character.<br /><b>issue_date</b> must be 6 digits of form YYMMDD, where YY is, unlike the doc_id, really the last two digits of the year, e.g. 00 is 2000, MM the 0-padded month, e.g. 05 for May, and DD the 0-padded day of the month.<br />* or % may expand to fill remaining digits, <b>but do not use ? and do not put * or % before a digit in issue_date</b><br /><b>title</b> must be less than 36 characters.<br /><b>synopsis</b> must be less than 66 characters.
  </p>
<div class="short_text">
     <label for="tempidx">Use MariaDB regular expressions</label> <input type="checkbox" class="form-checkbox"  name="regexp" id="regexp" '.$regexp.' /><br />Uncheck to use wildcards<br /><a href="https://mariadb.com/kb/en/regular-expressions-overview/" target="_blank">MariaDB regular expression syntax</a> &nbsp; <a href="/site/regex_query_suggestions.txt" target="_blank">regex suggestions</a>
</div>
  <div class="short_text">
